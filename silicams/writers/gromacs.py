...
class GromacsTopologyWriter(StructureWriter):
    ...
    def write_full_slit(
        ...
        OSError
            Raised when the staged ITP/TOP pair cannot be promoted or prior
            outputs cannot be restored after a promotion failure.

        Notes
        -----
        The ITP and TOP files are rendered before either final path is replaced,
        then promoted as one exception-safe output set.
        """
        if self._snapshot.source_kind != "silica_slit":
        ...
        itp_text = render_itp(atomtype_order, molecule_type)
        top_text = render_top(
        ...
        itp_path = Path(itp_link)
        top_path = Path(top_link)
        with staged_output_paths((itp_path, top_path)) as staging_paths:
            staging_paths[itp_path].write_text(itp_text, encoding="utf-8")
            staging_paths[top_path].write_text(
                top_text,
                encoding="utf-8",
            )
